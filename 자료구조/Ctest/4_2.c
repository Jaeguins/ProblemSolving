...
void createPoly(char* name) {
    ...
    strcpy_s(polys[n]->name, MAX_BUFFER_LENGTH, _strdup(name));
    polys[n++]->nbr_terms = 0;
}
...
void addTerm(char* name, int coef, int expo) {
    ...
    if (!isDup) {
        polys[index]->terms[polys[index]->nbr_terms] = malloc(sizeof(Term));
...
void diffPoly(char* name, char* result) {
    ...
    for (int i = 0; i < polys[index]->nbr_terms; i++) {
        addTerm(result,
            (polys[index]->terms[i]->expo*polys[index]->terms[i]->coef),
            (polys[index]->terms[i]->expo - 1)
            );
